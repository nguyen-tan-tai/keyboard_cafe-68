...
BleComboKeyboard::BleComboKeyboard(String deviceName, String deviceManufacturer, uint8_t batteryLevel)
  : hid(0) {
...
void BleComboKeyboard::taskServer(void* pvParameter) {
  BleComboKeyboard* kb = (BleComboKeyboard*)pvParameter;  //static_cast<BleComboKeyboard *>(pvParameter);
  BLEDevice::init(kb->deviceName);
  BLEServer* pServer = BLEDevice::createServer();
  pServer->setCallbacks(kb->connectionStatus);

  kb->hid = new BLEHIDDevice(pServer);
  kb->inputKeyboard = kb->hid->inputReport(KEYBOARD_ID);  // <-- input REPORTID from report map
  kb->outputKeyboard = kb->hid->outputReport(KEYBOARD_ID);
  kb->inputMediaKeys = kb->hid->inputReport(MEDIA_KEYS_ID);
  kb->connectionStatus->inputKeyboard = kb->inputKeyboard;
  kb->connectionStatus->outputKeyboard = kb->outputKeyboard;

  kb->inputMouse = kb->hid->inputReport(MOUSE_ID);  // <-- input REPORTID from report map
  kb->connectionStatus->inputMouse = kb->inputMouse;

  kb->outputKeyboard->setCallbacks(new KeyboardOutputCallbacks());

  kb->hid->manufacturer()->setValue(kb->deviceManufacturer);

  kb->hid->pnp(0x02, 0xe502, 0xa111, 0x0210);
  kb->hid->hidInfo(0x00, 0x01);

  BLESecurity* pSecurity = new BLESecurity();

  pSecurity->setAuthenticationMode(ESP_LE_AUTH_BOND);

  kb->hid->reportMap((uint8_t*)_hidReportDescriptor, sizeof(_hidReportDescriptor));
  kb->hid->startServices();

  BLEAdvertising* pAdvertising = pServer->getAdvertising();
  pAdvertising->setAppearance(HID_KEYBOARD);
  pAdvertising->addServiceUUID(kb->hid->hidService()->getUUID());
  pAdvertising->start();
  kb->hid->setBatteryLevel(kb->batteryLevel);
